        ...
        with open(data_path, encoding='utf-8') as reader:
            for i,line in enumerate(reader):
                ...
                for l in [1,101,201]:
                    for key in ['B','C']:
                        guid = "%s-%s" % (set_type,i)
                        text_a = tokenization.convert_to_unicode(lines["A"].replace("\n","")[-(l+250):-l])
                        text_b = tokenization.convert_to_unicode(lines[key].replace("\n","")[-(l+250):-l])
            ...
            for i,line in enumerate(reader):
                ...
                for l in [1,101,201]:
                    for key in ['B','C']:
                        ...
                        text_b = tokenization.convert_to_unicode(lines[key].replace("\n","")[-(l+250):-l])
                        label = tokenization.convert_to_unicode(key)
                        examples.append([guid, split_len, text_a, text_b, label])
        examples = pd.DataFrame(examples,columns=['guid','split_len','text_a','text_b',"label"])
        return  examples
...
def main(_):
  ...
  if FLAGS.do_predict:
    ...
    aa = test_id.iloc[test_id.groupby(['guid','split_len'])['pred'].idxmax()]

    aa = aa.groupby(["guid","label"]).count().reset_index()[['guid','label',"pred"]]

    pred_label = aa.iloc[aa.groupby("guid")["pred"].idxmax(),:]['label']
...
python main_new.py \
  ...
  --learning_rate=2e-5 \
  --num_train_epochs=10.0 \
  --output_dir=../tmp/main_new_big_data_maxavg_epochs_10_lr2e-5/
