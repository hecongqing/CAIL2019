...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    print("asdbnkasdbaksdsaasd")
    print("tensor_a",tensor_a)
    print("tensor_b",tensor_b)
    x1_norm = tf.sqrt(tf.reduce_sum(tf.square(tensor_a), axis=1))
    print("x1_norm",x1_norm)
    x2_norm = tf.sqrt(tf.reduce_sum(tf.square(tensor_b), axis=1))
    print("x2_norm",x2_norm)
    x1_x2 = tf.reduce_sum(tf.multiply(tensor_a, tensor_b), axis=1)
    print("asdbnkasdbaksdsaasd")
    print(tf.shape(x1_x2))
    print(tf.shape(x1_norm * x2_norm))
    output_layer = x1_x2 / (x1_norm * x2_norm)
    print("asdbnkasdbaksdsaasd")
    print(output_layer)
