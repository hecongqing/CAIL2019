...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    tensor_a = tf.slice(output_layer, [0, 0, 0],[tf.shape(output_layer)[0], tf.shape(output_layer)[1]//2,tf.shape(output_layer)[2]])
    mid =  tf.shape(output_layer)[1] - (tf.shape(output_layer)[1]//2)
    tensor_b = tf.slice(output_layer, [0, mid, 0],[tf.shape(output_layer)[0], mid, tf.shape(output_layer)[2]])

    distance = tf.sqrt(tf.reduce_sum(tf.square(tf.subtract(tensor_a,tensor_b)),1,keep_dims=True))
    distance =tf.div(distance,        tf.add(tf.sqrt(tf.reduce_sum(tf.square(tensor_a),1,keep_dims=True)),tf.sqrt(tf.reduce_sum(tf.square(tensor_b),1,keep_dims=True))))
    output_layer = tf.reshape(distance, [-1,tf.shape(distance)[-1]], name="distance")    
...
export BERT_BASE_DIR=./chinese_L-12_H-768_A-12
CUDA_VISIBLE_DEVICES=0 python main-Copy2.py \
  --task_name=task3 \
  --do_train=True \
  --train_path=../input/SCM_5k.json \
  --do_eval=False \
  ...
  --learning_rate=2e-5 \
  --num_train_epochs=10.0 \
  --output_dir=../tmp/maincopy2_big_data_pool_epochs_10_lr2e-5/
